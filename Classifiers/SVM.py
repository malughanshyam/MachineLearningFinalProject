...
if __name__ == '__main__':
    fileName = 'main/dataset_5_balanced_handpicked_features.csv'
...
#     df = MultiColumnLabelEncoder(columns = ['Auction','Make','Transmission','WheelType','Nationality','Size','TopThreeAmericanName','VNST']).fit_transform(df)
    ...
    df = MultiColumnLabelEncoder(columns = ['Auction','Make','Transmission','WheelType','Nationality','Size','TopThreeAmericanName']).fit_transform(df)
    ...
    svm = LinearSVC(C=1)
    builtModel = svm.fit(X_train, y_train)
